...
| **Denial of service** | Alert flooding · denial of service against the API or event bus | Rate limiting via Redis · alert consolidation / network case grouping reduces alert volume at the source (§1.9) · Redis Streams consumer groups sized with 5× headroom (§37, §38) |
